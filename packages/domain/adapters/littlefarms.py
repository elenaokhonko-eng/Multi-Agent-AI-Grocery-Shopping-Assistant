...
class LittleFarmsAdapter(RetailerAdapter):
    ...
    def get_checkout_details(self) -> dict[str, str]:
        ...
        user = os.environ.get("LITTLEFARMS_EMAIL", "")
        if not user:
            return {"address": "Not logged in (Missing LITTLEFARMS_EMAIL in .env)", "payment_method": "Not logged in"}
        ...
        try:
            with sync_playwright() as p:
                ...
                browser.close()
                return {"address": final_address, "payment_method": payment_text}
        except Exception as e:
            logger.error(f"[{self.store_name} Adapter] Error scraping account details: {e}")
            return {"address": f"Error scraping address for {user}", "payment_method": "Error scraping payment method"}
        ...
        try:
            with sync_playwright() as p:
                ...
                for item in items:
                    ...
                    if url:
                        logger.info(
                            f"[{self.store_name} Adapter] Adding {item.get('title', 'item')} to cart from {url}"
                        )
                        page.goto(url)
                        page.wait_for_timeout(3000)
                        # Execute JS to add to cart to avoid visibility issues
                        page.evaluate(
                            "if(document.querySelector('form[data-role=\"tocart-form\"]')) { document.querySelector('form[data-role=\"tocart-form\"]').submit(); }"
                        )
                        page.wait_for_timeout(4000)
                ...
                page.evaluate(
                    "if(document.querySelector('button.continue')) { document.querySelector('button.continue').click(); }"
                )
                ...
                page.evaluate(
                    f"if(document.querySelector('textarea#order_comment')) {{ document.querySelector('textarea#order_comment').value = '{comment}'; document.querySelector('textarea#order_comment').dispatchEvent(new Event('change')); }}"
                )
                ...
                logger.info(
                    f"[{self.store_name} Adapter] [SAFETY STOP]: Order would have been placed here. Uncomment code to go live."
                )
