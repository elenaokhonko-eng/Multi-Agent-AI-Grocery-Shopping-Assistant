...
from packages.retailers.shengsiong.page_objects import ShengSiongPageObject
...
class ShengSiongAdapter(RetailerAdapter):
    ...
    def __init__(self, session_profile_dir: str | None = None):
        ...
        self._page_object = ShengSiongPageObject()
    ...
    async def search_candidates(self, query: str, category_hint: str | None = None) -> list[CandidateProduct]:
        ...
        try:
            ...
            logger.warning("Sheng Siong page object search failed: %s", e)

        # 2. Driver Search Fallback
        if not candidates:
            try:
                driver_items = await self._live_driver.search_shengsiong(clean_query)
                ...
                logger.debug("Sheng Siong driver search failed: %s", e)
    ...
    async def submit_order(self, approval_token: str, slot_id: str = "") -> OrderConfirmation:
        ...
        if not live_enabled or self.retailer_id not in allowlist:
            raise NotImplementedError(
                ...
                "Set LIVE_PURCHASE_ENABLED=true and include 'shengsiong' in LIVE_PURCHASE_RETAILER_ALLOWLIST."
            )
        ...
        return OrderConfirmation(
            ...
            order_receipt_url=f"https://allforyou.sg/orders/{order_num}",
            gross_total_cents=cart.gross_total_cents,
            slot_id=slot_id or (self._selected_slot.slot_id if self._selected_slot else "slot_ss_1"),
