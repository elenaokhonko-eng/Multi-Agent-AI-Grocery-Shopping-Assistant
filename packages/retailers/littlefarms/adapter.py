...
class LittleFarmsAdapter(RetailerAdapter):
    ...
    async def submit_order(self, approval_token: str, slot_id: str = "") -> OrderConfirmation:
        ...
        slot_label = self._selected_slot.display_label if self._selected_slot else "Little Farms Standard"

        live_enabled = os.getenv("LIVE_PURCHASE_ENABLED", "false").lower() == "true"
        if not live_enabled:
            return OrderConfirmation(
                retailer_order_id=f"LF-DEMO-{uuid4().hex[:6].upper()}",
                confirmed_total_cents=cart.gross_total_cents,
                delivery_slot=slot_label,
                receipt_url="https://littlefarms.com/orders/demo",
                is_uncertain=True,
        ...
        return OrderConfirmation(
            ...
            confirmed_total_cents=cart.gross_total_cents,
            delivery_slot=slot_label,
            receipt_url=f"https://littlefarms.com/orders/{order_num}",
            is_uncertain=False,
            placed_at=datetime.now(UTC),
        )
